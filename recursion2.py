...
def groupSum(start, nums, target):
    if (start >= len(nums)):  # baseCase
        return (target == 0)
    else:
        return groupSum(start + 1, nums, target - nums[start]) or groupSum(start + 1,
                                                                           nums, target)
...
def groupSum6(start, nums, target):
    if (start >= len(nums)):
        # base case; boolean -- if index is greater than number of elemets
        return (target == 0)
    else:
        # if list contains 6, move to the next index and subtract 6 from target
        if nums[start] != 6:
            return groupSum6(start + 1, nums,
                             target - nums[start]) or groupSum6(start + 1, nums, target)
        # if list does not 6, move to next index and subtract value from target
        elif nums[start] == 6:
...
def groupNoAdj(start, nums, target):
    ...
    if (start >= len(nums)):
        return (target == 0)
    # see if there is an adjecent number used or not; index respectively
    else:
...
def groupSum5(start, nums, target):
    ...
    if nums[start] % 5 == 0:  # if number is multiple of 5
        return groupSum5(start + 1, nums, target - nums[start])  # choose the 5

    if nums[start] == 1 and nums[start - 1] % 5 == 0:  # if a 1 follows a multiple of 5
...
def groupSumClump(start, nums, target):
    ...
    if (start >= len(nums)):
        ...
        sum_clump = 1
        idx = 0
        while sum_clump < len(nums):
            if nums[idx] == nums[idx + 1]:
                sum_clump += 1
                idx += 1
            else:
                break
        return groupSumClump(start + sum_clump, nums, target - (nums[start] * sum_clump)) or groupSumClump(
            start + sum_clump, nums, target)
...
def splitArrayHelper(nums, idx, left, right):
    if not nums[idx:]:  # base case! if list is empty []
        return left == right

    return (splitArrayHelper(nums, idx + 1, left + nums[idx], right) or
            splitArrayHelper(nums, idx + 1, left, right + nums[idx]))
...
def splitOdd10Helper(nums, idx, left, right):
    if not nums[idx:]:  # base case! empty list []
        if (left % 10 == 0 and right % 2 == 1 or
                left % 2 == 1 and right % 10 == 0):
            return True
        else:
            return False

    return (splitOdd10Helper(nums, idx + 1, left + nums[idx], right) or
            splitOdd10Helper(nums, idx + 1, left, right + nums[idx]))
...
def split53Helper(nums, idx, left, right):
    # left contains 3s, right contains 5s
    if not nums[idx:]:  # base case
        return left == right

    if nums[idx] % 3 == 0:
        # multiples of 3 go in the left group
        return split53Helper(nums, idx + 1, left + nums[idx], right)
    elif nums[idx] % 5 == 0:
        # multiples of 5 go in the right group
        return split53Helper(nums, idx + 1, left, right + nums[idx])
    else:
        # same old splitArray method like earlier
        return (splitArrayHelper(nums, idx + 1, left + nums[idx], right) or
                splitArrayHelper(nums, idx + 1, left, right + nums[idx]))
